...
import com.study.disgroupportal.data.DataSource.getRequestById
...
fun AddRequestScreen(
    ...
    var request by remember {
        mutableStateOf(
            Request(
                authorName = mainVm.user?.name?.run { toString() } ?: "",
                authorId = mainVm.user?.id?.run { toString() } ?: ""
            )
        )
    }
    ...
    LaunchedEffect(Unit) {
        DisGroupPortalApp.curScreen = ADD_REQUEST
